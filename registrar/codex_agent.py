"""Codex Agent Identity 注册（核心库）。

通过 ChatGPT accessToken + Ed25519 密钥对，调用 /v1/agent/register
获取 agent_runtime_id，生成 Codex CLI 可用的 auth.json。
绕过 add-phone 限制，不走 OAuth 流程。

原始脚本 by 久雾，集成适配 by DangoMeow。
"""
from __future__ import annotations

import base64
import hashlib
import json
import logging
import re
import time
from pathlib import Path
from typing import Any

from cryptography.hazmat.primitives.asymmetric.ed25519 import Ed25519PrivateKey
from cryptography.hazmat.primitives.serialization import (
    Encoding,
    PrivateFormat,
    PublicFormat,
    NoEncryption,
    load_der_private_key,
)
from nacl.public import PrivateKey, SealedBox

logger = logging.getLogger(__name__)

AUTHAPI_BASE = "https://auth.openai.com/api/accounts"
AGENT_VERSION = "0.138.0-alpha.6"
AGENT_HARNESS_ID = "codex-cli"
RUNNING_LOCATION = "local"


def generate_ed25519_keypair() -> tuple[str, str]:
    """生成 Ed25519 密钥对。返回 (private_key_pkcs8_base64, public_key_ssh)。"""
    private_key = Ed25519PrivateKey.generate()

    pkcs8_der = private_key.private_bytes(
        encoding=Encoding.DER,
        format=PrivateFormat.PKCS8,
        encryption_algorithm=NoEncryption(),
    )
    private_key_b64 = base64.b64encode(pkcs8_der).decode()

    pub_bytes = private_key.public_key().public_bytes(
        encoding=Encoding.Raw,
        format=PublicFormat.Raw,
    )
    ssh_header = b"ssh-ed25519"
    blob = bytearray()
    blob.extend(len(ssh_header).to_bytes(4, "big"))
    blob.extend(ssh_header)
    blob.extend(len(pub_bytes).to_bytes(4, "big"))
    blob.extend(pub_bytes)
    public_key_ssh = f"ssh-ed25519 {base64.b64encode(bytes(blob)).decode()}"

    return private_key_b64, public_key_ssh


def register_codex_agent(
    session,
    access_token: str,
    public_key_ssh: str,
) -> str:
    """调用 /v1/agent/register 注册 agent，返回 agent_runtime_id。"""
    resp = session.post(
        f"{AUTHAPI_BASE}/v1/agent/register",
        headers={
            "Content-Type": "application/json",
            "Authorization": f"Bearer {access_token}",
        },
        json={
            "abom": {
                "agent_version": AGENT_VERSION,
                "agent_harness_id": AGENT_HARNESS_ID,
                "running_location": RUNNING_LOCATION,
            },
            "agent_public_key": public_key_ssh,
        },
        timeout=15,
    )
    if resp.status_code != 200:
        raise RuntimeError(f"Agent register failed: HTTP {resp.status_code} {resp.text[:300]}")

    data = resp.json()
    agent_runtime_id = data.get("agent_runtime_id")
    if not agent_runtime_id:
        raise RuntimeError(f"No agent_runtime_id in response: {data}")
    return agent_runtime_id


def decrypt_agent_task_id(private_key_b64: str, encrypted_task_id: str) -> str:
    """按 sub2api/Codex Agent Identity 规则把 encrypted_task_id 解成 task_id。"""
    der = base64.b64decode(private_key_b64)
    key = load_der_private_key(der, password=None)
    seed = key.private_bytes(
        encoding=Encoding.Raw,
        format=PrivateFormat.Raw,
        encryption_algorithm=NoEncryption(),
    )
    curve_private = bytearray(hashlib.sha512(seed).digest()[:32])
    curve_private[0] &= 248
    curve_private[31] &= 127
    curve_private[31] |= 64
    plaintext = SealedBox(PrivateKey(bytes(curve_private))).decrypt(
        base64.b64decode(encrypted_task_id)
    )
    task_id = plaintext.decode().strip()
    if not task_id:
        raise RuntimeError("empty decrypted task_id")
    return task_id


def register_codex_agent_task(
    session,
    access_token: str,
    agent_runtime_id: str,
    private_key_b64: str,
) -> str:
    """注册并返回明文 task_id，避免导入 sub2api 后首次请求才注册 task。"""
    der = base64.b64decode(private_key_b64)
    private_key = load_der_private_key(der, password=None)
    timestamp = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
    signature = private_key.sign(f"{agent_runtime_id}:{timestamp}".encode())
    resp = session.post(
        f"{AUTHAPI_BASE}/v1/agent/{agent_runtime_id}/task/register",
        headers={
            "Content-Type": "application/json",
            "Authorization": f"Bearer {access_token}",
        },
        json={
            "timestamp": timestamp,
            "signature": base64.b64encode(signature).decode(),
        },
        timeout=15,
    )
    if resp.status_code != 200:
        raise RuntimeError(f"Agent task register failed: HTTP {resp.status_code} {resp.text[:300]}")
    data = resp.json()
    task_id = data.get("task_id") or data.get("taskId")
    if task_id:
        return str(task_id).strip()
    encrypted_task_id = data.get("encrypted_task_id") or data.get("encryptedTaskId")
    if encrypted_task_id:
        return decrypt_agent_task_id(private_key_b64, str(encrypted_task_id))
    raise RuntimeError(f"No task_id in response: {data}")


def decode_jwt_claims(jwt_token: str) -> dict[str, Any]:
    """解码 JWT payload（不验证签名）。"""
    parts = jwt_token.split(".")
    if len(parts) != 3:
        raise ValueError("Invalid JWT format")
    payload_b64 = parts[1]
    padding = 4 - len(payload_b64) % 4
    if padding != 4:
        payload_b64 += "=" * padding
    return json.loads(base64.urlsafe_b64decode(payload_b64))


def extract_account_info(access_token: str) -> dict[str, str]:
    """从 accessToken JWT 提取 account_id, user_id, email, plan_type。"""
    claims = decode_jwt_claims(access_token)
    auth_info = claims.get("https://api.openai.com/auth", {})
    profile = claims.get("https://api.openai.com/profile", {})
    return {
        "account_id": auth_info.get("chatgpt_account_id", ""),
        "user_id": auth_info.get("chatgpt_user_id", ""),
        "email": profile.get("email", ""),
        "plan_type": auth_info.get("chatgpt_plan_type", "free"),
    }


def build_auth_json(
    agent_runtime_id: str,
    private_key_b64: str,
    account_id: str,
    user_id: str,
    email: str,
    plan_type: str = "free",
    task_id: str = "",
) -> dict[str, Any]:
    """组装 Codex CLI 的 auth.json。"""
    agent_identity = {
        "agent_runtime_id": agent_runtime_id,
        "agent_private_key": private_key_b64,
        "account_id": account_id,
        "chatgpt_user_id": user_id,
        "email": email,
        "plan_type": plan_type,
        "chatgpt_account_is_fedramp": False,
    }
    if task_id:
        agent_identity["task_id"] = task_id
    return {
        "auth_mode": "agent_identity",
        "agent_identity": agent_identity,
    }


def build_auth_json_from_credential(cred: dict) -> dict[str, Any]:
    """从注册结果 dict 组装 Codex CLI auth.json。"""
    extra = cred.get("extra") or {}
    stored = extra.get("codex_auth_json") or cred.get("codex_auth_json")
    if isinstance(stored, dict):
        return stored

    agent_runtime_id = cred.get("agent_runtime_id") or extra.get("agent_runtime_id") or ""
    agent_private_key = cred.get("agent_private_key") or extra.get("agent_private_key") or ""
    task_id = cred.get("agent_task_id") or cred.get("task_id") or extra.get("agent_task_id") or extra.get("task_id") or ""
    if not agent_runtime_id or not agent_private_key:
        raise ValueError("missing agent identity")

    account_id = ""
    user_id = ""
    plan_type = "free"
    email = str(cred.get("email") or "").strip()
    access_token = str(cred.get("access_token") or "").strip()
    if access_token:
        try:
            info = extract_account_info(access_token)
            account_id = info.get("account_id", "")
            user_id = info.get("user_id", "")
            email = info.get("email", "") or email
            plan_type = info.get("plan_type", "free")
        except Exception:
            pass

    return build_auth_json(
        agent_runtime_id=agent_runtime_id,
        private_key_b64=agent_private_key,
        account_id=account_id,
        user_id=user_id,
        email=email,
        plan_type=plan_type,
        task_id=task_id,
    )


def save_auth_json_copy(cred: dict, output_dir: str | Path | None = None) -> str:
    """把注册成功后的 Agent Identity auth.json 留存到本地，并回写到 cred。"""
    auth_json = build_auth_json_from_credential(cred)
    ident = auth_json.get("agent_identity", {})
    email = str(ident.get("email") or cred.get("email") or "codex-agent").strip()
    agent_runtime_id = str(ident.get("agent_runtime_id") or cred.get("agent_runtime_id") or "").strip()
    base_name = email or agent_runtime_id or "codex-agent"
    safe_name = re.sub(r"[^A-Za-z0-9_.@-]+", "_", base_name).strip("._") or "codex-agent"

    out_dir = Path(output_dir) if output_dir is not None else Path(__file__).resolve().parent / "data" / "codex_auth"
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path = out_dir / f"{safe_name}.auth.json"
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(auth_json, f, ensure_ascii=False, indent=2)

    cred["codex_auth_json"] = auth_json
    cred["codex_auth_path"] = str(out_path)
    return str(out_path)
