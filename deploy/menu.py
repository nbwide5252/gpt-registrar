...
def action_setup_wizard():
    ...
    for n in['smsbower','herosms']:
        ...
        nk=Q('请输入 '+n+' API Key',k[:6]+'...'if k and len(k)>4 else'')
        if nk:p['api_key']=nk
    ...
    L()
    D('用于接收 OpenAI 验证邮件')
    D('需要 Cloudflare Worker 部署 email-worker')
    print()
    worker=os.environ.get('CF_MAIL_API_URL','')or Q('邮箱 Worker URL','https://zhidexiu-mail.ppu2812859729.workers.dev')
    domain=os.environ.get('CF_MAIL_DOMAIN','')or Q('邮箱域名','zhidexiu.com')
    os.environ['CF_MAIL_API_URL']=worker;os.environ['CF_MAIL_DOMAIN']=domain
    D('检查 Worker 连通性...')
    try:
        r=req.get(worker+'/api/health',timeout=10)
        if r.status_code==200:G('Worker 在线')
        else:W('Worker 异常: '+str(r.status_code))
    except:W('Worker 连接失败')
    ...
    try:
        ...
        for n,pv in pvs.items():
            if not pv.get('api_key'):continue
            ...
            if bal is not None:
                ...
                G(n+' 余额: $'+str(round(bal,2))+' (约'+str(est)+'个)')
            cl=prov.get_countries_with_prices()
            if cl:countries=[c for c in cl if c['price']<=j['max_price']][:15];break
    except:pass
    if countries:
        print()
        ...
        if sel and sel.isdigit():
            idx=int(sel)-1
            if 0<=idx<len(countries):os.environ['SMS_COUNTRY']=countries[idx]['code']
    else:os.environ['SMS_COUNTRY']=os.environ.get('SMS_COUNTRY','52')
    ...
    u=Q('Sub2 地址',s2.get('url',''))
    if u:
        s2['url']=u
        s2['email']=Q('管理员邮箱',s2.get('email',''))
        s2['password']=Q('密码',s2.get('password',''))
        s2['default_group']=Q('分组名',s2.get('default_group','chatgpt1'))or'chatgpt1'
        s2f.write_text(json.dumps({'sub2api':s2},indent=2,ensure_ascii=False),encoding='utf-8')
        os.environ['SUB2_URL']=s2['url']
        os.environ['SUB2_EMAIL']=s2['email']
        os.environ['SUB2_PASSWORD']=s2['password']
        os.environ['SUB2_GROUP']=s2['default_group']
